...
void page_fault_handler(struct Env * curenv, uint32 fault_va)
{
	...
	if(curenv->ptr_pageWorkingSet[curenv->page_last_WS_index].empty != 0)
	{
		...
		uint32 ret = pf_read_env_page(curenv, (void *) fault_va);
		...
		while(1)
		{
			...
			for(int i=0;i<curenv->page_WS_max_size;i++)
			{
				...
				uint32 va = curenv->ptr_pageWorkingSet[curenv->page_last_WS_index].virtual_address;
 				...
 				struct Frame_Info* ptr_frame_info1 = get_frame_info(curenv->env_page_directory, (void*)va, &ptr_table1);
 				//cprintf("frame \n");
 				uint32 *ptr_page_table = NULL;
				get_page_table(curenv->env_page_directory, (void *)va, &ptr_page_table);
				if(((ptr_page_table[PTX(va)]&PERM_USED)==0) && ((ptr_page_table[PTX(va)]&PERM_MODIFIED)==0))
				{


					if((ptr_page_table[PTX(va)]&PERM_MODIFIED)!=0)
					{
						pf_update_env_page(curenv, (void *)va, ptr_frame_info1);
					}
					unmap_frame(curenv->env_page_directory,(void *) va);
					//cprintf("unmap \n");
					int ret = allocate_frame(&ptr_frame_info1) ;
					map_frame(curenv->env_page_directory, ptr_frame_info1, (void*)fault_va, PERM_USER|PERM_WRITEABLE);
			...
			for(int i=0;i<curenv->page_WS_max_size;i++)
			{
				...
				uint32 *ptr_page_table = NULL;
				get_page_table(curenv->env_page_directory, (void *)va, &ptr_page_table);
				if(((ptr_page_table[PTX(va)]&PERM_USED)==0) && ((ptr_page_table[PTX(va)]&PERM_MODIFIED)==PERM_MODIFIED))
				{
				...
				{
					ptr_page_table[PTX(va)]&=(~PERM_USED);
				}
...
void __page_fault_handler_with_buffering(struct Env * curenv, uint32 fault_va)
{
	...
	if(curenv->ptr_pageWorkingSet[curenv->page_last_WS_index].empty != 0)
		{
		    //cprintf(" Hello placement  \n");
	        struct Frame_Info * Frame_info;
 	        uint32 *ptr_page5 = NULL;
	        get_page_table(curenv->env_page_directory,(void*) fault_va,&ptr_page5);
	        Frame_info = get_frame_info(curenv->env_page_directory, (void*)fault_va, &ptr_page5);
	        if(((ptr_page5[PTX(fault_va)])&PERM_BUFFERED) != 0)
			{
	        	//cprintf(" Hello placement 1111  \n ");

					Frame_info->isBuffered=0;
					if(((ptr_page5[PTX(fault_va)])&PERM_MODIFIED) != 0)
					{
					...
					ptr_page5[PTX(fault_va)]&=(~PERM_BUFFERED);
					ptr_page5[PTX(fault_va)]|=(PERM_PRESENT);
	        ...
	        else
	        {
				...
				allocate_frame(&Frame_info);
				map_frame(curenv->env_page_directory,Frame_info,(void*)fault_va,PERM_USER|PERM_WRITEABLE);
				//cprintf(" Hello placement 2222222 \n");
				uint32 ret = pf_read_env_page(curenv, (void *) fault_va);
				//cprintf(" Hello placement 333333 \n");
	        	/*allocate_frame(&Frame_info);
				map_frame(curenv->env_page_directory,Frame_info,curenv->env_page_directory,PERM_USER|PERM_WRITEABLE);
				uint32 ret = pf_read_env_page(curenv, (void *) fault_va);*/
				...
				//cprintf(" Hello placement 444444444  \n");
			...
			while(1)
			{
				...
				for(int i=0;i<curenv->page_WS_max_size;i++)
				{
					...
					if(((ptr_page_table[PTX(va)]&PERM_USED)==0) && ((ptr_page_table[PTX(va)]&PERM_MODIFIED)==0))
					{
						//cprintf("Hello ifffffffffffffffffffffffffffffffffff 1  \n");
						found = 1;
						break;
					}
					curenv->page_last_WS_index++;
			...
			uint32 va = curenv->ptr_pageWorkingSet[curenv->page_last_WS_index].virtual_address;
			get_page_table(curenv->env_page_directory, (void *)va, &ptr_page_table);
			struct Frame_Info* ptr_frame_info1 = get_frame_info(curenv->env_page_directory, (void*)va, &ptr_page_table);

			if((ptr_page_table[PTX(va)]&PERM_MODIFIED)!=0)
			{

				LIST_INSERT_TAIL(&modified_frame_list,ptr_frame_info1);
			...
			{
				LIST_INSERT_TAIL(&free_frame_list,ptr_frame_info1);
			}
			//curenv->page_last_WS_index++;
			//curenv->ptr_pageWorkingSet[curenv->page_last_WS_index].virtual_address=fault_va;
			ptr_page_table[PTX(va)]|=PERM_BUFFERED;
			ptr_page_table[PTX(va)]&=(~PERM_PRESENT);
			ptr_frame_info1->isBuffered=1;
			get_page_table(curenv->env_page_directory,(void*)fault_va,&ptr_page_table);
			ptr_frame_info1=get_frame_info(curenv->env_page_directory, (void*)fault_va, &ptr_page_table);
			if(LIST_SIZE(&modified_frame_list)==getModifiedBufferLength())
			{
				...
				LIST_FOREACH(ptr_frame_info , &modified_frame_list)
				{
					...
					pf_update_env_page(ptr_frame_info->environment,(void *)ptr_frame_info->va,ptr_frame_info);
					get_page_table(ptr_frame_info->environment->env_page_directory,(void*)ptr_frame_info->va,&ptr_page_table);
					ptr_page_table[PTX(fault_va)]&=(~PERM_MODIFIED);
					LIST_REMOVE(&modified_frame_list,ptr_frame_info);
					LIST_INSERT_TAIL(&free_frame_list,ptr_frame_info);
			...
			if(((ptr_page_table[PTX(fault_va)])&PERM_BUFFERED) != 0)
			{
				ptr_frame_info1->isBuffered=0;
				if(((ptr_page_table[PTX(fault_va)])&PERM_MODIFIED) != 0)
				{
					LIST_REMOVE(&modified_frame_list,ptr_frame_info1);
				}
				else
				{
					LIST_REMOVE(&free_frame_list , ptr_frame_info1);
				}
			...
			{
				//int frame_idx = ptr_page_table[PTX(fault_va)] >> 12;
				//ptr_frame_info1->isBuffered = 0;

				int r = allocate_frame(&ptr_frame_info1);
				//if(r != E_NO_MEM)
				//{

					if(ptr_frame_info1->isBuffered  != 0)
					{
					   get_page_table(ptr_frame_info1->environment->env_page_directory,(void*)ptr_frame_info1->va,&ptr_page_table);
					   ptr_page_table[PTX(ptr_frame_info1->va)] &= ~(PERM_BUFFERED);

					   //unmap_frame(ptr_frame_info1->environment->env_page_directory,(void *) ptr_frame_info1->va);

					}
					ptr_frame_info1->isBuffered=0;
					r= map_frame(curenv->env_page_directory,ptr_frame_info1,(void*)fault_va,PERM_USER|PERM_WRITEABLE);
					pf_read_env_page(curenv,(void*)fault_va);
					/*if(pf_read_env_page(curenv, (void *)fault_va) == E_PAGE_NOT_EXIST_IN_PF)
					{
						cprintf(" Hello Replacement  \n");
						...
						{
							//cprintf("%x,%x",USTACKBOTTOM,USTACKTOP);
							//print_trapframe(&(curenv->env_tf));
							unmap_frame(curenv->env_page_directory,(void *) fault_va);
							panic("Page not found");
						}
					...
					ptr_frame_info1->environment = curenv;
					ptr_frame_info1->va = fault_va;
				}
